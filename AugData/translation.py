...
def randomword_augment(data_source, data_target, textcol="text"):
    ...
    for txt in tqdm(train_text):
        list_trans = translate(txt)

        if list_trans is not None and json.loads(list_trans)['errorCode'] == 0:
            en = get_reuslt(list_trans)
            ch_trans = translate(en)
            if ch_trans is not None and json.loads(ch_trans)['errorCode'] == 0:
                print(get_reuslt(ch_trans))
                augtxts1.append(get_reuslt(ch_trans))
                augtxts2.append(get_reuslt(ch_trans))
            else:
                print("error")
                augtxts1.append(str(txt))
                augtxts2.append(str(txt))
        else:
            print("error")
            augtxts1.append(str(txt))
            augtxts2.append(str(txt))
